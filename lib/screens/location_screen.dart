...
class LocationScreenState extends State<LocationScreen> {
  ...
  //   });
  // }
  void updateUI(dynamic weatherData, [String? city]) {
    setState(() {
      double temp = weatherData['main']['temp'];
      temperature = temp.toInt();
                  ...
                  children: <Widget>[
                    ...
                    GestureDetector(
                      onTap: () async {
                        ...
                        var weatherData = await getWeatherByLocation(
                            location.latitude, location.longitude);
                        String lat = location.latitude.toStringAsFixed(2);
                        String lon = location.longitude.toStringAsFixed(2);
                        updateUI(weatherData, 'Lat: $lat, Lon: $lon');
                      },
                      child: Image.asset(
                    ...
                    GestureDetector(
                      onTap: () async {
                        ...
                        if (typedCity != null) {
                          var weatherData = await getCityWeather(typedCity);
